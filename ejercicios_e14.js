...
const total = productos.reduce((acum, p) => acum + (p.precio*p.stock), 0);
console.log(total);
...
const categoelectrodomestico = productos.some(p => p.categoria == "Electrodomésticos");
console.log(categoelectrodomestico);
...
const ordemayor = productos.sort((a,b ) => a.precio - b.precio);
console.log(ordemayor);
//8. Crea una función flecha que reciba el arreglo y retorne los productos
//ordenados por nombre en orden alfabético inverso.
const ordenaralfa = productos.sort((a,b ) => b.nombre.localeCompare (a.nombre));
...
const promedio = productos.reduce((acum, p) => acum + p.precio, 0) / productos.length;
